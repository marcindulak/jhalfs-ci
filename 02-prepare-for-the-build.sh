...
# Suggested to LFS in https://lists.linuxfromscratch.org/sympa/arc/alfs-discuss/2025-08/msg00000.html
# LFS switched to ftpmirror.gnu.org after https://lists.linuxfromscratch.org/sympa/arc/lfs-dev/2025-12/msg00008.html
...
# Use a reliable mirror for the remaining download attempts, since ftpmirror.gnu.org will keep selecting the same broken mirror for the given package
su - vagrant -c 'source /vagrant/jhalfs/jhalfs.sh && cd /home/vagrant/jhalfs && set -x && sed -i "s/ftp.gnu.org/ftpmirror.gnu.org/" common/libs/func_download_pkgs'
su - vagrant -c 'source /vagrant/jhalfs/jhalfs.sh && cd /home/vagrant/jhalfs && set -x && sed -i "s/# GNU_REPLACE is not set/GNU_REPLACE=\"\$GNU_REPLACE\"/" configuration'
...
su - vagrant -c 'source /vagrant/jhalfs/jhalfs.sh && cd /home/vagrant/jhalfs && while ! $(curl -s --max-time 5 -L www.linuxfromscratch.org > /dev/null); do echo Trying to reach www.linuxfromscratch.org ...  && sleep 5; done && yes yes | GNU_REPLACE=mirrors.ibiblio.org/gnu ./jhalfs run'
...
su - vagrant -c 'source /vagrant/jhalfs/jhalfs.sh && cd /home/vagrant/jhalfs && while ! $(curl -s --max-time 5 -L www.linuxfromscratch.org > /dev/null); do echo Trying to reach www.linuxfromscratch.org ...  && sleep 5; done && yes yes | GNU_REPLACE=ftp.fau.de/gnu ./jhalfs run'
su - vagrant -c 'source /vagrant/jhalfs/jhalfs.sh && MISSING_FILES_DMP=$LFS/sources/MISSING_FILES.DMP && echo Try 4: $MISSING_FILES_DMP && cat $MISSING_FILES_DMP'
su - vagrant -c 'source /vagrant/jhalfs/jhalfs.sh && cd /home/vagrant/jhalfs && while ! $(curl -s --max-time 5 -L www.linuxfromscratch.org > /dev/null); do echo Trying to reach www.linuxfromscratch.org ...  && sleep 5; done && yes yes | GNU_REPLACE=mirrors.ocf.berkeley.edu/gnu ./jhalfs run'
su - vagrant -c 'source /vagrant/jhalfs/jhalfs.sh && MISSING_FILES_DMP=$LFS/sources/MISSING_FILES.DMP && echo Try 5: $MISSING_FILES_DMP && cat $MISSING_FILES_DMP'
su - vagrant -c 'source /vagrant/jhalfs/jhalfs.sh && cd /home/vagrant/jhalfs && while ! $(curl -s --max-time 5 -L www.linuxfromscratch.org > /dev/null); do echo Trying to reach www.linuxfromscratch.org ...  && sleep 5; done && yes yes | GNU_REPLACE=ftp.gnu.org/gnu ./jhalfs run'
su - vagrant -c 'source /vagrant/jhalfs/jhalfs.sh && MISSING_FILES_DMP=$LFS/sources/MISSING_FILES.DMP && echo Try 6: $MISSING_FILES_DMP && cat $MISSING_FILES_DMP'
