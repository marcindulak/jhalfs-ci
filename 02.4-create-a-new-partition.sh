...
# /vagrant synced-folder cannot be used for loop devices, but docker allowed for this until sime time like 2025-06 (Ubuntu 24.04)
# The current fix inside of docker is to use mknod to explicitly create the next available device
# losetup: /vagrant/build_dir.img: failed to set up loop device: No such file or directory
LOOPDEV=$(losetup -f)
[ -e "$LOOPDEV" ] || mknod -m660 "$LOOPDEV" b 7 "${LOOPDEV#/dev/loop}"
losetup "$LOOPDEV" /vagrant/build_dir.img
